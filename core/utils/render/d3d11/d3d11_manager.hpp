...
namespace dx11 {
	...
	struct d3d11_manager : draw_manager {
		...
		void* create_texture(uint32_t width, uint32_t height) override;

		ID3D11ShaderResourceView* get_texture(const uint32_t width, const uint32_t height);
		bool set_texture_rgba(tex_id id, const uint8_t* rgba, uint32_t width,
			uint32_t height) override;
		bool set_texture_rgba(ID3D11ShaderResourceView* id, const uint8_t* rgba, const uint32_t width, const uint32_t height);
		bool set_texture_rabg(const tex_id id, const uint8_t* rabg, const uint32_t width, const uint32_t height);
	...
	public:
		tex_dict_dx11* _tex_dict = nullptr;
