...
#include "..\datatypes\cutlhash\utllinkedlist.h"
...
public:
	...
	CConVar* find(uint32_t uHashedName)
	{
		for (int i = listConvars.Head(); i != listConvars.InvalidIndex(); i = listConvars.Next(i))
		{
			...
			if ( hash_32_fnv1a_const(pConVar->szName) == uHashedName)
				return pConVar;
		}
#ifdef _DEBUG
		debug(false); // invalid convar name
#endif
		return nullptr;
	}
