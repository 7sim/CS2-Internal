...
namespace edited
{
    ...
    void ImageSkin(const char* skin_name, ImTextureID user_texture_id, const ImVec2& size, ImU32 rarity_col, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
    {
        ...
        window->DrawList->AddShadowRect(bb.Min, bb.Max, rarity_col, 24.f, ImVec2(0, 0), ImDrawFlags_RoundCornersAll, 3.f);
        window->DrawList->AddRectFilled(bb.Min, bb.Max, VGetColorU32(ImColor(5, 5, 5, 255), 1.f), 0.0f);

        window->DrawList->AddImage(user_texture_id, bb.Min, bb.Max, uv0, uv1, GetColorU32(tint_col));

        // Define the rectangle area at the bottom of the image
        float rect_height = 30; // Adjust the height of the filled rectangle as needed
        ImVec2 rect_min = ImVec2(bb.Min.x, bb.Max.y - rect_height);
        ImVec2 rect_max = bb.Max;

        window->DrawList->AddRectFilled(rect_min, rect_max, VGetColorU32(ImColor(15, 15, 15, 255), 1.f), 0.0f);


        // Calculate the position for the text to be centered inside the filled rectangle
        ImVec2 text_size = ImGui::CalcTextSize(skin_name);
        ImVec2 text_pos = ImVec2(
            rect_min.x + (rect_max.x - rect_min.x - text_size.x) / 2,
            rect_min.y + (rect_max.y - rect_min.y - text_size.y) / 2
        );

        // Draw the text
        window->DrawList->AddText(text_pos, GetColorU32(ImVec4(1.0f, 1.0f, 1.0f, 1.0f)), skin_name); // Using white color for the text
    }
    bool ImageSkinButton(const char* skin_name, ImTextureID texture_id, const ImVec2& size, const ImVec4& bg_col, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
    {
        ...
        ImGuiID id = window->GetID(skin_name);
        ...
        window->DrawList->AddShadowRect(bb.Min, bb.Max, VGetColorU32(bg_col, 1.f), 24.f, ImVec2(0, 0), ImDrawFlags_RoundCornersAll, 3.f);
        window->DrawList->AddRectFilled(bb.Min, bb.Max, VGetColorU32(ImColor(5, 5, 5, 255), 1.f), 0.0f);

        // Render the image
        window->DrawList->AddImage(texture_id, bb.Min + padding, bb.Max - padding, uv0, uv1, GetColorU32(ImVec4(1.f, 1.f, 1.f, 0.7f)));

        if (hovered)
        {
            // Draw shadow rectangle covering the whole image
            ImVec4 shadow_col = ImVec4(0.f, 0.f, 0.f, 0.5f); // Adjust the color and alpha as needed
            window->DrawList->AddShadowRect(bb.Min, bb.Max, GetColorU32(shadow_col), 24.f, ImVec2(0, 0));

        }
        // Define the rectangle area at the bottom of the image
        float rect_height = 15.F; // Adjust the height of the filled rectangle as needed
        ImVec2 rect_min = ImVec2(bb.Min.x, bb.Max.y - rect_height);
        ImVec2 rect_max = bb.Max;
    ...
    bool ButtonPlus(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, ImTextureID plus_texture, bool selected)
    {
        ImGuiWindow* window = GetCurrentWindow();
        if (window->SkipItems)
            return false;
        ...
        // Calculate shadow outline parameters
        float shadow_width = 3.0f;
        ImU32 shadow_color = hovered ? VGetColorU32(c::accent, 0.6f) : VGetColorU32(c::accent, 0.1f);

        // Render shadow outline when pressed or hovered
        if (held) {
            shadow_color = VGetColorU32(c::accent, 1.f);
        }

        if (selected)
            shadow_color = VGetColorU32(c::accent, 0.9f);

        window->DrawList->AddShadowRect(bb.Min, bb.Max, shadow_color, 22.f, ImVec2(0, 0), ImDrawFlags_RoundCornersAll, shadow_width);

        // Render button background
        const ImU32 bg_color = GetColorU32((held && hovered) ? VGetColorU32(c::black, 1.0f) : hovered ? VGetColorU32(c::black, 1.f) : GetColorU32(c::bg::filling));
        window->DrawList->AddRectFilled(bb.Min, bb.Max, bg_color, 3.f);

        // Render frame
        const ImU32 frame_color = GetColorU32((held && hovered) ? VGetColorU32(c::black, 1.0f) : hovered ? VGetColorU32(c::black, 1.f) : GetColorU32(c::bg::filling));
        RenderFrame(bb.Min, bb.Max, frame_color, true, 3.f);

        // Calculate image position to center it within the button
        ImVec2 image_size(20, 20); // Set the size of your cross/plus image
        ImVec2 image_pos = bb.Min + (size - image_size) * 0.5f;

        // Render the cross/plus image
        if (plus_texture)
            window->DrawList->AddImage(plus_texture, image_pos, image_pos + image_size, ImVec2(0, 0), ImVec2(1, 1), GetColorU32(c::black));
    ...
    bool Pages(bool selected, const char* label, ImTextureID image_icon, const ImVec2& size_arg)
    {
        ...
        ImVec4 checkedColor(74.0f / 255.0f, 166.0f / 255.0f, 219.0f / 255.0f, 1.0f); // #4AA6DB
        state.text = ImLerp(state.text, selected ? c::element::text_active : c::element::text, g.IO.DeltaTime * 6.f);
        state.icon = ImLerp(state.icon, selected ? checkedColor : c::element::text, g.IO.DeltaTime * 6.f);
...
#include <string>
    ...
    bool Keybind(const char* label, int* key, int* mode, bool* show)
    {
        ...
        edited::CheckboxWithPopup(false, "Show In Binds", "-", show);
    ...
    bool BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, ImGuiChildFlags child_flags, ImGuiWindowFlags window_flags)
    {
        ...
        // Çocuk alanýnýn üst kýsmýný doldurma
        GetWindowDrawList()->AddRectFilled(parent_window->DC.CursorPos - ImVec2(0, 35), parent_window->DC.CursorPos + ImVec2(size_arg.x - 0.4f, 0), GetColorU32(checkedColor), c::child::rounding, ImDrawFlags_RoundCornersTop);
        ...
        const bool ret = Begin(temp_window_name, NULL, window_flags | ImGuiWindowFlags_AlwaysVerticalScrollbar);
    ...
    struct check_state
    {
        std::vector<std::string> pageTexts = { "Copy", "Description", "New Bind", "Colors", "Reset" };
    ...
    bool CheckboxWithPopup(bool popup_box, const char* label, const char* hint, bool* v, int* key, int* mode, bool* value, bool keybind, bool picker, Color_t* color, ImGuiColorEditFlags flags)
    {
        ImGuiWindow* window = GetCurrentWindow();
        if (window->SkipItems) return false;
        ...
        ImVec4 checkedColor(74.0f / 255.0f, 166.0f / 255.0f, 219.0f / 255.0f, 1.0f); // #4AA6DB
        state.background = ImLerp(state.background, *v ? checkedColor : c::element::filling, g.IO.DeltaTime * 12.f);
        state.circle = ImLerp(state.circle, *v ? checkedColor : c::element::circle_mark, g.IO.DeltaTime * 12.f);
        ...
        if (popup_box)
        {
            ...
            if (picker) RenderTextColor(font::icomoon, rect.Min, rect.Max - ImVec2(keybind ? 75 : 47, 0), ImColor(color->GetVec4()), "e", ImVec2(1.0, 0.49f));
                    ...
                    for (int i = 0; i < state.pageTexts.size(); i++)
                    {
                        ...
                        if (ImGui::BeginMenuEx(state.pageTexts[i].c_str(), state.icons[i].c_str()))
                        {
                            ...
                            if (i == 3 && picker) {
                                ...
                                edited::ColorPicker4("picker", arrColor, flags);
                                {
    ...
    bool SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
    {
        ...
        if (grab_bb.Max.x > grab_bb.Min.x) {
            ...
            ImVec4 checkedColor(74.0f / 255.0f, 166.0f / 255.0f, 219.0f / 255.0f, 1.0f); // #4AA6DB
            GetWindowDrawList()->AddRectFilledMultiColor(slider_bb.Min + ImVec2(0, 1), ImVec2(state.slow + frame_bb.Min.x - 3 - state.circle_pump, grab_bb.Max.y + 1), GetColorU32(checkedColor), GetColorU32(c::black), GetColorU32(c::black), GetColorU32(checkedColor), 30.f);
    ...
    bool MultiCombo(const char* szLabel, unsigned int* pFlags, const char* const* arrItems, int nItemsCount)
    {
        ...
        if (BeginCombo(szLabel, strBuffer.c_str(), nItemsCount, ImGuiComboFlags_None, NULL))
        {
            PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(15, 15));

            for (int i = 0; i < nItemsCount; i++)
            {
                const bool item_selected = (*pFlags & (1 << i));


                const int nCurrentFlag = (1 << i);
                const char* name = arrItems[i];
                if (edited::Selectable(name, (*pFlags & nCurrentFlag), ImGuiSelectableFlags_::ImGuiSelectableFlags_DontClosePopups))
                {
                    // flip bitflag
                    *pFlags ^= nCurrentFlag;
                    bValueChanged = true;
    ...
    bool Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
    {
        ...
        ImVec4 checkedColor(74.0f / 255.0f, 166.0f / 255.0f, 219.0f / 255.0f, 1.0f); // #4AA6DB
        GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max + ImVec2(0, 1), GetColorU32(checkedColor, state.opticaly), 2.f);
